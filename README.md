...
<table>
 <tbody>
  ...
  <tr>
   <td>/crawlview/crawl/</td>
   <td>POST</td>
   <td>
    <li>트위터에서 crawl한 결과와 이들을 sentiment analysis한 결과 리턴하되, /crawlview/main/으로 리다이렉트 한 후 표시합니다.</li> 
    <li>/crawlview/main/에서 '키워드1'로 검색했다면, '키워드1'이 모든 트윗에 포함되어 있어야 합니다. 마찬가지로, /crawlview/main/에서 '키워드1 키워드2'로 검색했다면, '키워드1', '키워드2'가 모든 트윗에 포함되어 있어야 합니다.</li>
    ...
    <li>아래 사진은 /crawlview/main/에서 '불닭'으로 검색한 것으로, '불닭'이 모든 트윗에 포함되어 있음을 나타냅니다. negative 분류된 트윗 없어서 0개로 표시된 점 주의 바랍니다.</li>
  ...
  <tr>
   <td>/crawlview/commit/</td>
   <td>POST</td>
   <td>
    <li>/crawlview/main/에서 '키워드1'로 입력했다면, positive 분류된 트윗을 database 내 '키워드1_positive' table, negative 분류된 트윗을 database 내 '키워드1_negative' table에 적재합니다. /crawlview/main/에서 '키워드1 키워드2'로 입력했다면, database 내 '키워드1_키워드2_positive', '키워드1_키워드2_negative' 테이블에 적재합니다.</li> 
  ...
  <tr>
   ...
   <td>GET</td>
   <td>
    <li>데이터베이스 저장된 테이블 리스트 출력 명령 입력받거나, 혹은 데이터베이스 저장된 특정 테이블 내용 출력하도록 table name 입력받는 화면을 리턴합니다.</li> 
    <li>'all' 입력받은 경우, 데이터베이스 저장된 테이블 리스트 표시합니다. '키워드1_positve', '키워드1_negative', '키워드1_키워드2_positve', '키워드1_키워드2_negative' 입력하여 해당 테이블 내용을 불러올 수 있습니다. 이 때, 모든 row를 데이터베이스에서 웹서버 불러오되 화면에는 100개 row만 표시합니다. 데이터를 데이터베이스에서 불러오는 시간인 search time도 표시합니다.</li>
  ...
  <tr>
   ...
   <td>POST</td>
   <td>
    <li>/dbview/main/에서 'all' 입력받은 경우, 데이터베이스에 있는 테이블 리스트 리턴하되, /dbview/main/ 리다이렉트 해서 표시합니다.</li>
    <li>'키워드1_positve', '키워드1_negative', '키워드1_키워드2_positve', '키워드1_키워드2_negative' 입력받아 해당 테이블 내용 리턴하되, /dbview/main/ 리다이렉트 해서 표시합니다. 데이터를 데이터베이스에서 불러오는 시간인 search time도 표시합니다.</li>
...
- requirement 구현 시 제약사항으로 인해 django ORM 아닌 raw sql 사용하였기에, sql injection 방지할 필요 있었습니다. /crawlview/main/, /dbview/main/에서 입력받은 것 그대로 이용해서 database에서 sql execute 하는 경우 있기에 주의가 필요합니다.
  - /crawlview/main/에서 건네받은 키워드가, /crawlview/crawl/에서 한국어, 일본어 아닌 것이 확인되면 raise exception 하였습니다. 한국어, 일본어로만으로는 sql injection 일으킬 수 없기 때문에, 한국어, 일본어만 valid keyword 인정한다는 논리입니다.
  - /dbview/main/에서 건네받은 키워드가, /dbview/search/에서 '키워드1_positive', '키워드1_negative', '키워드1_키워드2_positive', '키워드1_키워드2_negative' 의 형식을 엄격히 지키지 않는 경우 raise exception 하였습니다.
