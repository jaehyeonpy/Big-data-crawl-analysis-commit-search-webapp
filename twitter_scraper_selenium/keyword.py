...
class Keyword:
    ...
    def fetch_and_store_all(self):
        try:
            ...
            logger.exception(
                "Error at method fetch_and_store_all : {}".format(ex))

    def fetch_and_store_some(self):
        all_ready_fetched_posts = []
        present_tweets = Finder.find_all_tweets(self.driver) 
        self.check_tweets_presence(present_tweets) 
        all_ready_fetched_posts.extend(present_tweets)
        ...
        while len(self.posts_data) < self.tweets_count:
            for tweet in present_tweets:
                content = Finder.find_content(tweet)
                self.posts_data[len(self.posts_data)] = content

            logger.info('tweets fetched so far: %d' % len(self.posts_data)) 
    ...
    def scrap_all(self):
        try:
            ...
            self.close_driver()
            logger.exception(
                "Error at method scrap_all on : {}".format(ex))
...
def scrape_all_keyword(keyword: str, browser: str = "firefox", until: Union[str, None] = None,
...
def scrape_some_keyword(
    ...
    '''
        Output: 
            {0: (content 0), ...} for the expected output, 
            {} if no tweet found in twitter for the keyword,
            {} if abnormal tweets_count like 0, -1, ...
    '''
